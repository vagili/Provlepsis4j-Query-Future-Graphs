...
- It constructs **edge-level features** from the previously computed node embeddings using the **Hadamard product** and trains either **logistic regression** or **LASSO regression** to estimate, for each candidate edge, the probability that it will exist. A direct **TGN** prediction model is also available for the TGN family.
...
Each prediction timeline is identified by its embedding family and prediction model. In separate-database mode, databases use names such as `<base>-node2vec-logistic`, `<base>-node2vec-lasso`, `<base>-tgn-logistic`, `<base>-tgn-lasso`, and `<base>-tgn-tgn`. In single-database mode, the same family/model combinations are stored as independent relationship timelines.
...
- Provlepsis4j executes the query on both the **current** and the selected family/model future timeline and presents the results side by side, making differences explicit.
...
- It inserts the new predicted edges with an updated prediction round (`predicted_timestep`) while leaving all other timelines unchanged.
