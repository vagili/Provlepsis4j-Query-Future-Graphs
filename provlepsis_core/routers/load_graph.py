...
def _coerce_int_default(value: Any, default: int) -> int:
    ...
    inner_default = getattr(value, "default", default)

    try:
        return int(inner_default)
    except Exception:
        return int(default)
...
def _create_multi_edge_rels_query(rel_type: str) -> str:
    ...
    SET r += row.props
    SET r.predicted = 0
    SET r.edgeOrigin = 'observed'
    SET r.storageRepresentation = 'single_db_multi_edge'
    """
...
EXCLUDE_KEYS = [
    ...
    "rawTimestamp",
    "timePoint",
    "timePointStartTimestamp",
    "timePointEndTimestamp",
]
...
def _purge_stringish_props(db: str) -> Dict[str, Any]:
    ...
    if node_keys:
        rows = run_data(
            ...
            WHERE size(toDrop) > 0
            SET n += apoc.map.fromPairs([k IN toDrop | [k, null]])
            RETURN count(n) AS c
            """),
            {"keys": node_keys},
        )
        ...
        if rows is None:
            raise HTTPException(
                status_code=500,
                detail="APOC not available. Install APOC or disable purge_all_stringish.",
            )
    ...
    if rel_keys:
        rows = run_data(
            ...
            WHERE size(toDrop) > 0
            SET r += apoc.map.fromPairs([k IN toDrop | [k, null]])
            RETURN count(r) AS c
            """),
            {"keys": rel_keys},
        )
        ...
        rel_count = int(rows[0]["c"]) if rows else 0
...
# ====================================
# In-memory feature transformation
# ====================================
...
async def load_graph(
    ...
    storageRepresentation: str = Form(SEPARATE_DBS),
    maxTimeRelationshipTypes: int = Form(512),
):
    ...
    try:
        ...
        if storage_mode == SINGLE_DB_MULTI_EDGE:
            max_time_relationship_types = _coerce_int_default(
                maxTimeRelationshipTypes,
                512,
            )

            return _load_graph_single_db_multi_edge(
                new_db=new_db,
                edges=edges,
                features=features,
                batch_size=batch_size,
                max_categories=max_categories,
                purge_all_stringish=purge_all_stringish,
                isTemporal=isTemporal,
                timestampColumn=timestampColumn,
                max_time_relationship_types=max_time_relationship_types,
            )
        ...
        if purge_all_stringish:
            purge_summary = {
                ...
                "relAssignmentsDropped": 0,
                "note": (
                    "Skipped automatic string-property purge for single-db multi-edge import. "
                    "Multi-edge mode intentionally stores string metadata such as edgeOrigin, "
                    "baseRelType, storageRepresentation, and edgeUid."
                ),
            }
...
def _canonicalize_undirected_endpoints(
    df_e: pd.DataFrame,
    src_col: str,
    dst_col: str,
) -> pd.DataFrame:
...
def _drop_self_loops(
    ...
    out = df_e.loc[~same_endpoint].copy()
    return out, before - len(out)


def _load_graph_single_db_multi_edge(
    *,
    new_db: str,
    edges: UploadFile,
    features: Optional[UploadFile],
    batch_size: int,
    max_categories: int,
    purge_all_stringish: bool,
    isTemporal: bool,
    timestampColumn: str | None,
    max_time_relationship_types: int = 512,
) -> Dict[str, Any]:
    ...
    df_e = df_e.dropna(subset=[src_col, dst_col]).copy()
    df_e["_input_order"] = range(len(df_e))

    if type_col and type_col in df_e.columns:
        df_e["_base_type"] = df_e[type_col].astype(str).apply(_sanitize_type)
    else:
        df_e["_base_type"] = "REL"
    ...
    if timestampColumn and str(timestampColumn).strip():
        _lc_map = {c.lower(): c for c in df_e.columns}
        tcol_resolved = _lc_map.get(str(timestampColumn).strip().lower())

    if isTemporal and tcol_resolved:
        df_e["_timestamp"] = pd.to_numeric(df_e[tcol_resolved], errors="coerce")
        bad = int(df_e["_timestamp"].isna().sum())
        ...
        if bad > 0:
            raise HTTPException(
                status_code=400,
                detail=(
                    f"Timestamp column '{tcol_resolved}' contains {bad} non-numeric "
                    "or missing value(s). Clean it or disable temporal import."
                ),
            )
    ...
    # ------------------------------------------------------------
    # Remove self-loops; keep parallel edges
    # ------------------------------------------------------------
    df_e, self_loops_removed = _drop_self_loops(df_e, src_col, dst_col)
    df_e = _canonicalize_undirected_endpoints(df_e, src_col, dst_col)

    # ------------------------------------------------------------
    # Sort rows for deterministic relSequence assignment
    # ------------------------------------------------------------
    sort_cols = [src_col, dst_col, "_base_type"]

    if has_real_timestamp:
        df_e = df_e.sort_values(
            sort_cols + ["_timestamp", "_input_order"],
            kind="mergesort",
        )
    else:
        df_e = df_e.sort_values(
    ...
    df_e["rawTimestamp"] = df_e["_timestamp"].apply(
        lambda x: _canon_num(0 if pd.isna(x) else x)
    )

    (
        df_e["timePoint"],
        df_e["timePointStartTimestamp"],
        df_e["timePointEndTimestamp"],
        time_type_summary,
    ) = _assign_adaptive_time_points(
        df_e,
        raw_col="_timestamp",
        max_time_relationship_types=max_time_relationship_types,
    )
    ...
    df_e["timestamp"] = df_e["rawTimestamp"]


    df_e["relSequence"] = (
        df_e.groupby(
            [src_col, dst_col, "_base_type", "timePoint", "rawTimestamp"]
        ).cumcount() + 1
    )


    df_e["_rel_type"] = df_e.apply(
        lambda r: observed_rel_type(r["_base_type"], r["timePoint"]),
        axis=1,
    )

    df_e["firstTimestamp"] = df_e["timestamp"]
    df_e["lastTimestamp"] = df_e["timestamp"]
    df_e["timestamps"] = df_e["timestamp"].apply(lambda x: [x])
    df_e["interactionCount"] = 1
    df_e["baseRelType"] = df_e["_base_type"]
    df_e["edgeOrigin"] = "observed"
    df_e["storageRepresentation"] = SINGLE_DB_MULTI_EDGE
    df_e["hasRealTimestamp"] = bool(has_real_timestamp)

    df_e["edgeUid"] = df_e.apply(
        lambda r: (
            f"{r[src_col]}|{r[dst_col]}|{r['_base_type']}|"
            f"TP{r['timePoint']}|RAW{r['rawTimestamp']}|SEQ{int(r['relSequence'])}"
        ),
        axis=1,
    )

    print(
        "[LOAD DEBUG] multi-edge rows=",
        len(df_e),
        "distinct raw timestamps=",
        time_type_summary.get("distinctRawTimestamps"),
        "distinct relationship time points=",
        time_type_summary.get("distinctRelationshipTimePoints"),
        "compressed=",
        time_type_summary.get("compressed"),
        flush=True,
    )
    ...
    for chunk in _chunk(list(ids), batch_size):
        run(
            _with_db(new_db, _merge_nodes_query()),
            {"rows": _rows_for_nodes(chunk)},
        )
    ...
    for typ, grp in df_e.groupby("_rel_type"):
        rows = _rel_rows(grp)
        for chunk in _chunk(rows, batch_size):
            run(
                _with_db(new_db, _create_multi_edge_rels_query(typ)),
                {"rows": chunk},
            )
            rel_total += len(chunk)
    ...
    if features is not None:
        ...
        if "id" not in cols_lower:
            raise HTTPException(
                status_code=400,
                detail="Features CSV must have an 'id' column.",
            )
        ...
        for chunk in _chunk(rows, batch_size):
            run(
                _with_db(new_db, _set_props_query()),
                {"rows": chunk},
            )
...
def _assign_adaptive_time_points(
    df: pd.DataFrame,
    raw_col: str,
    max_time_relationship_types: int = 512,
) -> Tuple[pd.Series, pd.Series, pd.Series, Dict[str, Any]]:

    max_types = max(1, int(max_time_relationship_types or 512))

    raw_numeric = pd.to_numeric(df[raw_col], errors="coerce")
    unique_times = sorted(set(float(v) for v in raw_numeric.dropna().tolist()))

    if not unique_times:
        point = pd.Series([0] * len(df), index=df.index)
        start = pd.Series([0] * len(df), index=df.index)
        end = pd.Series([0] * len(df), index=df.index)

        return point, start, end, {
            "mode": "no_timestamp",
            "distinctRawTimestamps": 0,
            "maxTimeRelationshipTypes": max_types,
            "distinctRelationshipTimePoints": 1,
            "compressed": False,
        }


    if len(unique_times) <= max_types:
        raw_to_point = {
            t: _canon_num(t)
        ...
        point = raw_numeric.map(lambda x: raw_to_point.get(float(x), 0))
        start = raw_numeric.map(lambda x: raw_to_start.get(float(x), 0))
        end = raw_numeric.map(lambda x: raw_to_end.get(float(x), 0))

        return point, start, end, {
            "mode": "exact_timestamp",
            "distinctRawTimestamps": len(unique_times),
            "maxTimeRelationshipTypes": max_types,
            "distinctRelationshipTimePoints": len(unique_times),
            "compressed": False,
        }
    ...
    raw_to_bucket: Dict[float, int] = {}
    bucket_to_values: Dict[int, List[float]] = {}

    for i, t in enumerate(unique_times):
        bucket = min(k, int((i * k) / n) + 1)
        raw_to_bucket[t] = bucket
        bucket_to_values.setdefault(bucket, []).append(t)

    bucket_start = {
        b: _canon_num(min(vals))
        for b, vals in bucket_to_values.items()
    }

    bucket_end = {
        b: _canon_num(max(vals))
        for b, vals in bucket_to_values.items()
    }

    def _bucket_for(x: Any) -> int:
        try:
            return int(raw_to_bucket.get(float(x), 0))
        except Exception:
            return 0

    point = raw_numeric.map(_bucket_for)
    start = point.map(lambda b: bucket_start.get(int(b), 0))
    end = point.map(lambda b: bucket_end.get(int(b), 0))

    return point, start, end, {
        "mode": "adaptive_logical_snapshot",
        "distinctRawTimestamps": len(unique_times),
        "maxTimeRelationshipTypes": max_types,
        "distinctRelationshipTimePoints": len(bucket_to_values),
        "compressed": True,
        "firstRawTimestamp": _canon_num(unique_times[0]),
        "lastRawTimestamp": _canon_num(unique_times[-1]),
    }