...
def _autodiscover_node_props(labels: List[str]) -> List[str]:
    ...
    rows = run_data(f"""
        ...
        UNWIND ks AS k
        WITH DISTINCT k
        WHERE NOT k IN ['_id','_tmp','_ts','eid']
...
def _is_gds_supported_node_property_value(v: Any) -> bool:
...
def _filter_supported_node_props(props: List[str]) -> List[str]:
    ...
    for p in props:
        ...
        ok = True
        for r in rows:
            if not _is_gds_supported_node_property_value(r.get("v")):
                ok = False
                break
...
def _delete_all_split_endpoint_labels():

    try:
        rows = run_data("""
          CALL db.labels() YIELD label
          WHERE label STARTS WITH '__SPLIT_'
            AND label CONTAINS '_EP_'
          RETURN label
        """)
...
# =========================
def _drop_in_memory_gds_graph(name: Optional[str]):
    if not name:
        return
...
def _split_rel_count(reltype: Optional[str]) -> int:
    if not reltype:
        return 0
...
def _endpoint_label_count(label: Optional[str]) -> int:
    if not label:
        return 0
...
def _skipped_projection(graph_name: Optional[str], reason: str) -> Dict[str, Any]:
    return {
...
# ---------- temp-endpoint label projection ----------
...
def _project_with_label_only(
    ...
    if not graph_name:
        return _skipped_projection(graph_name, "No graph name was requested.")

    if not endpoint_label:
        return _skipped_projection(graph_name, "No endpoint label was provided.")

    if not rel_types:
        return _skipped_projection(graph_name, "No relationship types were provided.")

    endpoint_label = _safe_label(endpoint_label)

    if _endpoint_label_count(endpoint_label) == 0:
        _drop_in_memory_gds_graph(graph_name)
        return _skipped_projection(
            graph_name,
            f"Endpoint label '{endpoint_label}' has no nodes.",
        )

    existing_rel_types = [rt for rt in rel_types if _split_rel_count(rt) > 0]

    if not existing_rel_types:
        _drop_in_memory_gds_graph(graph_name)
        return _skipped_projection(
            graph_name,
            "None of the requested relationship types has relationships.",
        )

    node_projection = {
        endpoint_label: ({"properties": node_props} if node_props else {})
    }
    ...
    rows = run_data(
        ...
        """,
        {
            "name": graph_name,
            "nodeProjection": node_projection,
            "relationshipTypes": existing_rel_types,
        },
    )
...
def _materialize_temp_split_relationships(
    ...
    def _create_pairs(limit_k: int, reltype: str, exclude_types: List[str], batch_size: int = 5000) -> int:
        ...
        CALL {{
          ...
          ORDER BY apoc.util.md5([toString(s), toString(t), toString(typ)])
          LIMIT $k
          RETURN s,t,typ
        }}
        CALL {{
...
class ExecuteSplitBody(BaseModel):
    ...
    includePredictedFamily: Optional[str] = None
    includePredictedModel: Optional[str] = None
    includePredictedBeforeLevel: Optional[int] = None
    keepTemporarySplitRelationships: bool = False
...
# Route
# =========================
def _observed_multi_edge_where() -> str:
    where, _params = _multi_edge_source_where()
    return where

def _multi_edge_source_where(
    include_predicted_family: Optional[str] = None,
    include_predicted_model: Optional[str] = None,
    include_predicted_before_level: Optional[int] = None,
) -> Tuple[str, Dict[str, Any]]:
    if (
        include_predicted_family
        and include_predicted_model
        and include_predicted_before_level is not None
    ):
        return (
            """
            NOT type(r) STARTS WITH 'SPLIT_'
            AND (
              coalesce(r.edgeOrigin, 'observed') = 'observed'
              OR (
                coalesce(r.edgeOrigin, '') = 'predicted'
                AND coalesce(r.embeddingFamily, '') = $includePredictedFamily
                AND coalesce(
                      r.predictionModel,
                      'logistic_regression'
                    ) = $includePredictedModel
                AND toInteger(
                      coalesce(
                    ...
                    ) < $includePredictedBeforeLevel
              )
            )
            """,
            {
                "includePredictedFamily": include_predicted_family,
                "includePredictedModel": include_predicted_model,
                "includePredictedBeforeLevel": int(
                    include_predicted_before_level
                ),
            },
        )
...
def _materialize_multi_edge_static_split_relationships(
    test_holdout: float,
    val_holdout: Optional[float],
    types: _SplitTypes,
    batch_size: int = 5000,
    ...
    rows = run_data(f"""
        MATCH (a)-[r]-(b)
        WHERE {_observed_multi_edge_where()}
        WITH
          CASE WHEN id(a) < id(b) THEN id(a) ELSE id(b) END AS u,
          CASE WHEN id(a) < id(b) THEN id(b) ELSE id(a) END AS v,
          collect(DISTINCT toFloat(coalesce(r.timestamp, r.lastTimestamp, r.firstTimestamp, 0))) AS tsList
        RETURN DISTINCT u, v, tsList
        ORDER BY apoc.util.md5([toString(u), toString(v)])
    """)

    pairs = []
    for r in rows:
        ts = sorted(set(float(x) for x in (r.get("tsList") or [0.0])))
        if not ts:
            ts = [0.0]
    ...
    test_rows = pairs[:k_test]
    val_rows = pairs[k_test:k_test + k_val]
    train_rows = pairs[k_test + k_val:]
...
def _materialize_multi_edge_temporal_split_relationships(
    test_holdout: float,
    val_holdout: Optional[float],
    types: _SplitTypes,
    time_prop: str = "timestamp",
    batch_size: int = 5000,
    explicit_test_timestamps: Optional[List[float]] = None,
    explicit_val_timestamps: Optional[List[float]] = None,
    include_predicted_family: Optional[str] = None,
    ...
    source_where, source_params = _multi_edge_source_where(
        include_predicted_family=include_predicted_family,
        include_predicted_model=include_predicted_model,
        include_predicted_before_level=include_predicted_before_level,
    )
    ...
    rows = run_data(
        ...
        CASE
            WHEN r.timestamps IS NOT NULL
                ...
                AND size(r.timestamps) > 0
            THEN [x IN r.timestamps | toFloat(x)]
            ELSE [toFloat(coalesce(r.`{time_prop}`, r.timestamp, r.lastTimestamp, r.firstTimestamp, 0.0))]
        END AS rel_ts
        RETURN u, v, rel_ts
        """,
        source_params,
    ...
    if explicit_test_timestamps is not None:
        test_times = {float(x) for x in explicit_test_timestamps if float(x) in uniq_ts_set}
    else:
        n_test = int(ceil(len(uniq_ts) * max(0.0, min(1.0, float(test_holdout)))))
        n_test = min(n_test, len(uniq_ts))
        test_times = set(uniq_ts[-n_test:]) if n_test > 0 else set()
    ...
    if explicit_val_timestamps is not None:
        val_times = {float(x) for x in explicit_val_timestamps if float(x) in uniq_ts_set}
        val_times = val_times - test_times
    else:
        n_test = len(test_times)
        n_val = int(ceil(len(uniq_ts) * max(0.0, min(1.0, float(val_holdout or 0.0)))))
...
def execute_master_split(body: ExecuteSplitBody):
    ...
    _delete_all_split_endpoint_labels()
    ...
    try:
        ...
        if storage_mode == SINGLE_DB_MULTI_EDGE:
            if body.temporalMode:
                counts = _materialize_multi_edge_temporal_split_relationships(
                test_holdout=body.testHoldout,
                val_holdout=body.valHoldout if want_val else 0.0,
                types=types,
                time_prop=body.timeProperty or "timestamp",
                explicit_test_timestamps=body.explicitTestTimestamps,
                explicit_val_timestamps=body.explicitValTimestamps if want_val else [],
                include_predicted_family=body.includePredictedFamily,
                include_predicted_model=body.includePredictedModel,
                include_predicted_before_level=body.includePredictedBeforeLevel,
            )
            else:
                counts = _materialize_multi_edge_static_split_relationships(
                    test_holdout=body.testHoldout,
                    val_holdout=body.valHoldout if want_val else 0.0,
                    types=types,
                )
        ...
        # TEST optional
        test_info = None
        want_test = bool(body.testGraphName) and _split_rel_count(types["test"]) > 0
        ...
        if want_test:
            ...
            test_info = _skipped_projection(
                body.testGraphName,
                "No test graph requested or no test split relationships were created.",
            )

        # VAL optional
        val_info = None
        want_val_project = bool(body.valGraphName) and _split_rel_count(types["val"]) > 0
        ...
        if want_val_project:
            ...
            val_info = _skipped_projection(
                body.valGraphName,
                "Validation was requested, but no validation split relationships were created.",
            )
        ...
        if not body.keepTemporarySplitRelationships:
